...
class OnsetsAndFrames(nn.Module):
    def __init__(self, input_features, output_features, model_complexity=48):
        ...
        self.combinedtab_stack = nn.Sequential(
            sequence_model(output_features * 3, model_size),
            nn.Linear(model_size, 126),
            StringSoftmax()
        )
    ...
    def forward(self, mel):
        ...
        combinedtab_pred = self.combinedtab_stack(combined_pred)
        #velocity_pred = self.velocity_stack(mel)
        return onset_pred, offset_pred, activation_pred, frame_pred, tab_pred, combinedtab_pred#, velocity_pred
    ...
    def run_on_batch(self, batch):
        ...
        onset_pred, offset_pred, _, frame_pred, tab_pred, combinedtab_pred = self(mel)
        ...
        predictions = {
            ...
            'combinedtab' : combinedtab_pred#.reshape(*tab_label.shape)#,
            #'tab_binary' : tab_pred,
        ...
        losses = {
            ...
            'loss/tab' : F.binary_cross_entropy(predictions['tab'].view(-1, predictions['tab'].size(-1)), tab_label.view(-1, tab_label.size(-1)).float()),
            'loss/combinedtab' : F.binary_cross_entropy(predictions['combinedtab'].view(-1, predictions['combinedtab'].size(-1)), tab_label.view(-1, tab_label.size(-1)).float())
            #self.categorical_cross_entropy_by_string(predictions['combinedtab'], tab_label),
    ...
    def categorical_cross_entropy_by_string(self, tab_pred, tab_label):
        ...
        tab_pred_flat = tab_pred.view(-1, tab_pred.size(-1)).float()
        tab_label_flat = tab_label.view(-1, tab_label.size(-1)).float()
        loss = ce(tab_pred_flat, tab_label_flat)
        return loss
