...
class PianoRollAudioDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.sequence_length is not None:
            ...
            if(audio_length < self.sequence_length):
                print(f"audio_len={audio_length}, sequence_length={self.sequence_length}")
            ...
            #print(step_end-step_begin)
            #print(data['tab'].shape)
            #print(step_begin)
            cutTab = data['tab'][step_begin:step_end, :, :]
            """if(cutTab.shape[0] < 320):
                step_begin -= step_begin + n_steps - cutTab.shape[0]
                step_end -= step_begin + n_steps - cutTab.shape[0]"""
            #print(cutTab.shape)
            result['audio'] = data['audio'][begin:end].to(self.device)
            result['label'] = data['label'][step_begin:step_end, :].to(self.device)
            result['tab'] = data['tab'][step_begin:step_end, :, :].to(self.device) #coment this for older model evaluations
            #print(result['tab'].shape)
            
        else:
            result['audio'] = data['audio'].to(self.device)
            result['label'] = data['label'].to(self.device)
            result['tab'] = data['tab'].to(self.device).float()
    ...
    def load(self, audio_path, tsv_path, tab_path):
        """
        ...
        labelTab = torch.from_numpy(np.load(tab_path))
        ...
        data = dict(path=audio_path, audio=audio, label=label, velocity=velocity, tab=labelTab)
...
class GuitarSet(PianoRollAudioDataset):
    def __init__(self, path='data/GuitarSet', groups=None, sequence_length=None, seed=42, device=DEFAULT_DEVICE):
        super().__init__(path, groups if groups is not None else ['05'], sequence_length, seed, device)
...
class IDMT_SMT_frst_chords(PianoRollAudioDataset):
    def __init__(self, path='data/IDMT-SMT/dataset1/FenderStratCleanNeckSC/', groups=None, sequence_length=None, seed=42, device=DEFAULT_DEVICE):
        super().__init__(path, groups if groups is not None else ['00', '01'], sequence_length, seed, device)
...
class IDMT_SMT_scnd(PianoRollAudioDataset):
